...
def read_data(file_path, id, name):
    ...
    with open(file_path, 'r', encoding='utf-8-sig') as f:
        for idx, line in enumerate(f):
            ...
            # train_title.append(line[1] + ','.join(HanLP.extractSummary(text, 1)))
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if score > self.best:
            self.best = score
            self.early_stopping = 0
            model.save_weights('./model/bert{}.w'.format(fold))
        else:
...
def predict(data):
    ...
    for i in tqdm(range(len(val_x1))):
        ...
        t1, t1_ = tokenizer.encode(first=achievements[:MAX_LEN], second=requirements[:MAX_LEN])
        T1, T1_ = np.array([t1]), np.array([t1_])
        _prob = model.predict([T1, T1_])
        prob.append(_prob[0])
    return prob
...
for fold, (train_index, valid_index) in enumerate(skf.split(train_achievements, labels)):
    ...
    model.load_weights('./model/bert{}.w'.format(fold))
    oof_test += predict([test_achievements, test_requirements])
    K.clear_session()

oof_test /= 5
np.savetxt('./model/train_bert.txt', oof_train)
np.savetxt('./model/test_bert.txt', oof_test)

cv_score = 1.0 / (1 + mean_absolute_error(labels+1, np.argmax(oof_train, axis=1) + 1))
print(cv_score)
test['Level'] = np.argmax(oof_test, axis=1) + 1
test[['Guid', 'Level']].to_csv('../result/bert_{}.csv'.format(cv_score), index=False)
